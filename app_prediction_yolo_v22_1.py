...
# ------------------------------------------------------------
# Détection du QR (focus bas de page)
# ------------------------------------------------------------
def detect_bv_qr(img_rgb: np.ndarray):
    """
    Détecte le QR (candidate BV) et retourne (data, points).
    ...
    Retour:
      ...
      - points: (4,2) coins QR dans coordonnées page entière
    """
    if not HAS_CV2:
        return None, None
...
def crop_region_right_of_qr(img_rgb: np.ndarray, qr_points: np.ndarray):
    """
    Découpe un rectangle à droite du QR-code (zone Référence).
    """
    h, w, _ = img_rgb.shape
    xs = qr_points[:, 0]
    ys = qr_points[:, 1]
...
# ------------------------------------------------------------
# Référence : pattern strict (imprimé BV) + extraction OCR
# ------------------------------------------------------------
...
def extract_reference_from_text(text: str):
    """
    Cherche une référence au format :
      XX XXXXX XXXXX XXXXX XXXXX XXXXX
    dans le texte OCR (zone à droite du QR).
    """
    norm = (
        text.replace("\xa0", " ")
        .replace("\u202f", " ")
    ...
    # fallback: grosse séquence de chiffres, reformatée en 2 + 5*5
    digit_sequences = []
...
def is_reference_strict(ref: str | None) -> bool:
    if not ref:
        return False
    return re.fullmatch(REF_PATTERN_STRICT, re.sub(r"\s+", " ", ref).strip()) is not None
...
def extract_reference_from_qr_payload(txt: str):
    ...
    Retourne une référence formatée (QRR) ou brute (SCOR).
    """
    norm = txt.replace("\r", "\n")
    ...
    for i, line in enumerate(lines):
        if line in ("QRR", "SCOR"):
            ...
            if line == "QRR":
                ...
                # SCOR n'est pas forcément au format "XX XXXXX..."
                return ref_line or None
...
# ------------------------------------------------------------
# V22.1 : Comptage factures par BV VALIDÉ (QR + Référence stricte)
# ------------------------------------------------------------
def assign_invoices_by_bv(df_pages: pd.DataFrame, bv_flag_col: str = "has_bv"):
    """
    Même logique que v22, mais sur un flag bv validé (colonne bv_flag_col).

    Hypothèse:
      - Si une page contient un BV validé, alors c'est la DERNIÈRE page de la facture.
    """
    df = df_pages.copy()
    invoice_ids = []
    cur = 1
...
# ------------------------------------------------------------
st.set_page_config(
    page_title="BV v22.1 – Validation par Référence + Comptage factures",
    layout="wide",
)
st.title("🧾 BV v22.1 – BV validé = QR + Référence (format strict)")
...
st.markdown(
    """
**Nouveauté v22.1 (anti faux-positifs QR) :**

On ne considère **BV** une page que si :
1) on détecte un QR en bas de page (**candidate**), **ET**
2) on arrive à extraire une **Référence** au format strict :

> `XX XXXXX XXXXX XXXXX XXXXX XXXXX`

La référence peut venir :
- de l'OCR à droite du QR, ou
- en fallback du payload QR (QRR/SCOR) *si on peut la reformater en format strict*.

➡️ **Le comptage de factures** utilise uniquement `has_bv=True` (BV validé).
"""
)
...
model_path_text = st.text_input(
    "Chemin du modèle YOLO (.pt) – optionnel (tampons non utilisés pour le comptage v22.1)",
...
# ------------------------------------------------------------
# Pipeline principal v22.1
# ------------------------------------------------------------
if uploaded_pdf:
    tmp_dir = Path("data/tmp_bv_v22_1")
    ...
    for i in range(n_pages):
        ...
        if has_qr_candidate:
            ...
            if ref_zone_text.strip():
                ref_ocr = extract_reference_from_text(ref_zone_text)
                if ref_ocr and is_reference_strict(ref_ocr):
                    bv_reference = ref_ocr
                    ref_source = "OCR_DROITE_QR"

            # 3b) fallback via payload QR (QRR -> reformat)
            if (not bv_reference) and qr_text:
                ref_payload = extract_reference_from_qr_payload(qr_text)
                # IMPORTANT v22.1 : on valide UNIQUEMENT si format strict
        ...
        # 4) Monnaie + Montant (debug/contrôle)
        bv_currency = None
        bv_amount = None
        if qr_text:
            bv_currency, bv_amount = extract_currency_amount_from_qr_text(qr_text)

        # 5) VALIDATION BV (anti faux positifs)
        # Condition demandée : "BV = au moins un QR + une Référence au format strict"
    ...
    # --------------------------------------------------------
    # Comptage factures basé sur BV VALIDÉ (has_bv)
    # --------------------------------------------------------
    ...
    # AFFICHAGE
    # --------------------------------------------------------
    st.subheader("🧮 Comptage de factures (v22.1 : BV validé = QR + Référence strict)")
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Factures clôturées (BV validés)", invoices_closed)
    ...
    # Alerte faux-positifs QR: QR détecté mais pas de référence
    false_qr = df_pages2[(df_pages2["has_qr_candidate"]) & (~df_pages2["has_bv"])]
    if len(false_qr) > 0:
        st.warning(
            f"{len(false_qr)} page(s) ont un **QR détecté** en bas de page mais **pas de Référence strict** "
            f"→ elles sont ignorées comme BV (anti faux-positifs)."
        )
    ...
    st.subheader("📄 Résultats par page (QR candidate / Référence / BV validé / invoice_id)")
    st.dataframe(df_pages2, use_container_width=True)

    st.subheader("🧾 Résumé par facture (segmentation v22.1)")
    ...
    with c1:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"bv_v22_1_pages_{pdf_path.stem}.csv",
        )
    with c2:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"bv_v22_1_invoices_{pdf_path.stem}.csv",
        )
    with c3:
        st.download_button(
            "📥 Télécharger QR faux-positifs (CSV)",
            data=false_qr.to_csv(index=False).encode("utf-8") if len(false_qr) > 0 else "page\n".encode("utf-8"),
            mime="text/csv",
            file_name=f"bv_v22_1_false_qr_{pdf_path.stem}.csv",
        )

    # Debug : OCR zone Référence
    with st.expander("🔍 Debug Référence : texte OCR à droite du QR (par page)"):
        ...
        if st.button("Afficher le texte OCR de cette page (droite du QR)"):
            ...
            st.text(txt if txt.strip() else "[aucun texte / pas de QR candidate sur cette page]")
    ...
    st.info("Dépose un PDF multipages pour analyser BV + compter les factures (v22.1).")
