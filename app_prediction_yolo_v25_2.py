# app_prediction_yolo_v25_3_com.py
...
def crop_amount_regions_invoice(img_rgb: np.ndarray):
    ...
    # droite moitié haut/milieu (utile pour "CHF 330.-" à droite)
    crops.append(img_rgb[int(h * 0.25) : int(h * 0.78), int(w * 0.52) : w])

    # bande centrée autour de "Facture" (souvent le montant est à droite de ce bloc)
...
def crop_date_regions_top_third(img_rgb: np.ndarray):
    ...
    crops = []
    crops.append(top)
    crops.append(top[:, int(w * 0.45) : w])  # zone droite (cachet / "Ins, le ...")
        ...
        for line in lines:
            ...
            if mw:
                ...
                if mm and (1 <= d <= 31) and (1900 <= y <= 2099):
                    cand = _fmt_date_iso(d, mm, y)
                    score = 2 + boost + penalty + 2  # format "18 novembre 2024" prioritaire
                    if score > best_score:
                        best_score = score
                        best = cand
...
tmp_dir = Path("data/tmp_bv_v25_3_com")
...
if "df_final_pages" in st.session_state and "df_final_invoices" in st.session_state:
    ...
    with d1:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v25_3_pages_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d2:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v25_3_factures_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d3:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v25_3_qr_ignores_{pdf_path.stem}.csv",
            use_container_width=True,
        )
