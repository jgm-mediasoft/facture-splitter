...
# ------------------------------------------------------------
# OCR – Transformers (TrOCR + DeepSeek)
# ------------------------------------------------------------
try:
    import torch
...
DEEPSEEK_MODEL_NAME = "deepseek-ai/deepseek-vl-base"


# ------------------------------------------------------------
# Préparation / cadrage unique pour TOUS les OCR
# ------------------------------------------------------------
def prepare_image_for_ocr(img_rgb: np.ndarray) -> Image.Image:
    """
    Applique le même cadrage (crop) à l'image pour TOUS les moteurs OCR.
    - enlève ~2% de marges sur chaque côté
    - retourne une image PIL recadrée
    """
    h, w, _ = img_rgb.shape
...
def load_deepseek_model():
    ...
    if not HF_TRANSFORMERS_AVAILABLE:
        return None, None
    try:
        processor = AutoProcessor.from_pretrained(DEEPSEEK_MODEL_NAME)
        model = AutoModelForVision2Seq.from_pretrained(DEEPSEEK_MODEL_NAME)
        DEEPSEEK_READY = True
        return processor, model
    except Exception:
        DEEPSEEK_READY = False
...
# ------------------------------------------------------------
# Fonctions OCR (toutes reçoivent la même image recadrée)
# ------------------------------------------------------------
...
def ocr_with_deepseek(pil_img: Image.Image) -> str:
    if not HF_TRANSFORMERS_AVAILABLE:
        return ""
    ...
    if processor is None or model is None:
        return ""
    try:
        inputs = processor(images=pil_img, text="OCR", return_tensors="pt")
...
def run_ocr_best(img_rgb: np.ndarray, engine: str = "auto") -> str:
    """
    Choix du moteur OCR :
      - engine = "auto": TrOCR -> Tesseract -> DeepSeek
      - engine = "trocr": TrOCR -> Tesseract -> DeepSeek
      - engine = "tesseract": Tesseract -> TrOCR -> DeepSeek
      - engine = "deepseek": DeepSeek -> TrOCR -> Tesseract

    Tous les moteurs travaillent sur le MÊME cadrage (prepare_image_for_ocr).
    """
    engine = engine.lower()
    pil_img = prepare_image_for_ocr(img_rgb)

    if engine == "trocr":
        order = ["trocr", "tesseract", "deepseek"]
    elif engine == "tesseract":
        order = ["tesseract", "trocr", "deepseek"]
    elif engine == "deepseek":
        order = ["deepseek", "trocr", "tesseract"]
    else:  # auto
        order = ["trocr", "tesseract", "deepseek"]
    ...
    for eng in order:
        if eng == "trocr":
            text = ocr_with_trocr(pil_img)
        elif eng == "tesseract":
            text = ocr_with_tesseract(pil_img)
        elif eng == "deepseek":
            text = ocr_with_deepseek(pil_img)
        if text.strip():
            break
...
# Config Streamlit
# ------------------------------------------------------------
st.set_page_config(page_title="YOLO – Tampons & factures (v15)", layout="wide")
st.title("🧭 Détection de tampons + découpe des factures + BV (v15, OCR multi-moteurs)")

ocr_status_msgs = []
if HF_TRANSFORMERS_AVAILABLE:
    ocr_status_msgs.append("Transformers ✅")
else:
    ocr_status_msgs.append("Transformers ❌")

if TESSERACT_AVAILABLE:
    ocr_status_msgs.append("Tesseract ✅")
else:
    ocr_status_msgs.append("Tesseract ❌")

st.caption(" | ".join(ocr_status_msgs))
...
Nouveauté v15 : OCR multi-moteurs (TrOCR, Tesseract, DeepSeek) avec un **cadrage identique** pour tous,  
et détection BV par page et par facture via la présence de QR-code.
"""
)

# ------------------------------------------------------------
# Helpers montants & monnaie (fallback global facture)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# QR-code – détection + décodage
# ------------------------------------------------------------
def detect_qr(img_rgb: np.ndarray):
    """
    Retourne (has_qr, data).
    has_qr = True si un QR-code est détecté (points != None),
             data = chaîne décodée (peut être vide selon le PDF).
    """
    if not HAS_CV2:
        return False, None
...
def is_bv_page(text: str, img_rgb: np.ndarray) -> bool:
    """
    Page BV si :
      - QR-code détecté (has_qr = True), OU
      - mots-clés BV dans le texte OCR (fallback).
    """
    has_qr, _ = detect_qr(img_rgb)
    if has_qr:
        return True
...
ocr_engine_choice = st.selectbox(
    ...
    options=[
        ("auto", "Auto (TrOCR → Tesseract → DeepSeek)"),
        ("trocr", "HuggingFace TrOCR en priorité"),
        ("tesseract", "Tesseract en priorité"),
        ("deepseek", "DeepSeek en priorité"),
    ],
    format_func=lambda x: x[1],
)
ocr_engine = ocr_engine_choice[0]
...
if uploaded_pdf:
    ...
    for i in range(n_pages):
        ...
        text_ocr = run_ocr_best(img_rgb, engine=ocr_engine)
        page_texts.append(text_ocr)

        # Détection QR -> BV par page
        has_qr, _ = detect_qr(img_rgb)
        page_has_bv_list.append(bool(has_qr))
        ...
        rows.append(
            {
                ...
                "page_has_bv": bool(has_qr),  # ✅ BV par page
            }
        )
            ...
            data_invoices.append(
                {
                    ...
                    "montant": montant,
                    "monnaie": monnaie,
                    "has_bv": has_bv,      # ✅ BV pour la facture
                    "bv_reference": bv_ref,
                }
            )
    ...
    st.download_button(
        ...
        mime="text/csv",
        file_name=f"yolo_tampons_factures_v15_{pdf_path.stem}.csv",
    )
    ...
    if not df_factures.empty:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"factures_resume_v15_{pdf_path.stem}.csv",
        )
