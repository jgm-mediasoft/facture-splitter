# app_prediction_yolo_v26_com.py
...
def ocr_image_rgb_with_conf(img_rgb: np.ndarray, mode: str = "text"):
    ...
    try:
        ...
        if mode in ("amount", "date"):
            ...
            df = pytesseract.image_to_data(pil_img, lang=lang, config=cfg, output_type=pytesseract.Output.DATAFRAME)
        else:
...
def crop_date_regions_top_third(img_rgb: np.ndarray):
    ...
    crops = []
    crops.append(top)
    crops.append(top[:, int(w * 0.45) : w])
    crops.append(top[int(y_end * 0.15) : y_end, 0:w])
    return [c for c in crops if c.size > 0]
...
_CCY_AMOUNT_RX = re.compile(
    r"\b(CHF|EUR)\b[\s:]*([0-9][0-9 '\u2019.,\-]{0,20})",
    flags=re.IGNORECASE,
)
...
def extract_total_amount_from_ocr_text(text: str):
    ...
    for line in lines:
        ...
        if "tva" in low and "total" not in low:
            base_penalty = -1
        else:
            base_penalty = 0
...
def extract_invoice_date_from_top(img_rgb: np.ndarray):
    ...
    for crop in crop_date_regions_top_third(img_rgb):
        ...
        for line in lines:
            ...
            penalty = 0
            if "échéance" in low or "echeance" in low:
                penalty -= 6
...
tmp_dir = Path("data/tmp_bv_v26_com")
...
if "df_final_pages" in st.session_state and "df_final_invoices" in st.session_state:
    ...
    if "amount_confidence" in df_invoices.columns:
        df_invoices["amount_confidence"] = df_invoices["amount_confidence"].apply(
            lambda x: round(float(x), 3) if x is not None and pd.notna(x) else None
        )
    ...
    with d1:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v26_pages_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d2:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v26_factures_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d3:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v26_qr_ignores_{pdf_path.stem}.csv",
            use_container_width=True,
        )
