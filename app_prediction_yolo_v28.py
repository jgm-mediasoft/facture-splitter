# app_prediction_yolo_v28_com.py
# v28 = v27 STRICT après validation + scan rapide sans OCR avant validation
...
def compute_invoice_meta(df_invoices: pd.DataFrame, pdf_path: Path, dpi: int, read_amount: bool, read_date: bool):
    ...
    for _, row in inv.iterrows():
        ...
        if read_date and TESSERACT_AVAILABLE:
            try:
                img_first = render_pdf_page_to_rgb(pdf_path, page_index=page_start - 1, dpi=dpi)
...
tmp_dir = Path("data/tmp_bv_v28_com")
...
# ==========================================================
# ÉTAPE 1 (FAST) : créer df_pages_editable sans OCR
# ==========================================================
if "df_pages_editable" not in st.session_state or st.session_state.get("last_pdf_name") != uploaded_pdf.name:
    ...
    for i in range(n_pages):
        img_rgb = render_pdf_page_to_rgb(pdf_path, page_index=i, dpi=dpi_fast)
        ...
        # IMPORTANT : on n'a pas has_bv en FAST, on utilise un proxy pour pré-segmentation :
        # - has_bv_fast = QR candidate (à ajuster manuellement)
        row = {
            "page": i + 1,
            "has_qr_candidate": bool(has_qr_candidate),
            "has_reference_strict": False,
            "has_bv": bool(has_qr_candidate),  # proxy FAST uniquement pour pré-cochage invoice_start
            "bv_reference": None,
            "ref_source": None,
            "bv_currency": None,
            "bv_amount": None,
...
# ==========================================================
# ÉTAPE 2 (STRICT v27) : au clic recalc, on exécute la v27 telle quelle
# ==========================================================
if recalc:
...
# ==========================================================
# AFFICHAGE : STRICT v27 (copié/identique)
# ==========================================================
