...
def ocr_page_from_image(img_rgb: np.ndarray) -> str:
    """
...
# Config Streamlit
# ------------------------------------------------------------
st.set_page_config(page_title="YOLO – Tampons & factures (v13)", layout="wide")
st.title("🧭 Détection de tampons + découpe des factures + BV (v13)")
...
if not HAS_TESSERACT:
    st.warning(
        ...
        "Pour les factures SANS BV (par ex. 'CHF 330.-'), installe Tesseract pour extraire le texte."
    )
...
if uploaded_pdf:
    ...
    st.download_button(
        ...
        mime="text/csv",
        file_name=f"yolo_tampons_factures_v13_{pdf_path.stem}.csv",
    )
    ...
    if not df_factures.empty:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"factures_resume_v13_{pdf_path.stem}.csv",
        )
