# app_prediction_yolo_v25_1_com.py
...
def compute_invoice_amounts(df_invoices: pd.DataFrame, df_pages2: pd.DataFrame, pdf_path: Path, dpi: int):
    ...
    for _, row in inv.iterrows():
        ...
        if best_amt is not None:
            sources.append("FACT_OCR_NO_BV" if not has_bv else "FACT_OCR_BV_NO_AMOUNT")
        else:
            sources.append(None)
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### Montant (si BV sans montant / pas de BV)")
    read_amount_if_missing = st.checkbox(
        "Lire montant sur facture",
        value=True,
...
tmp_dir = Path("data/tmp_bv_v25_1_com")
...
if recalc:
    ...
    if read_amount_if_missing and TESSERACT_AVAILABLE and df_final_invoices.shape[0] > 0:
...
if "df_final_pages" in st.session_state and "df_final_invoices" in st.session_state:
    ...
    with d1:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v25_1_pages_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d2:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v25_1_factures_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d3:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v25_1_qr_ignores_{pdf_path.stem}.csv",
            use_container_width=True,
        )
