...
def prepare_image_for_ocr(img_rgb: np.ndarray) -> Image.Image:
    ...
    - enlève ~2% de marges sur chaque côté (évite les bords blancs)
    ...
    if bottom <= top or right <= left:
        # sécurité : pas assez grand, on garde l'image originale
        cropped = img_rgb
    else:
...
# ------------------------------------------------------------
# Fonctions OCR individuelles (toutes prennent une PIL image
# déjà recadrée par prepare_image_for_ocr)
# ------------------------------------------------------------
