# app_prediction_yolo_v30_com.py
# v30 = v29_1 + accélération extraction (sans changer tes paramètres UI)
# Accélérations clés:
# 1) Cache des rendus (page, dpi) => une page DPI 400 rendue au plus 1 fois
# 2) QR detect+decode uniquement en DPI_detect (phase 2) + points scalés pour DPI 400 (pas de re-détection en 400)
# 3) Référence BV: priorité au payload QRR (0 OCR si OK), OCR seulement en fallback
# 4) Montant facture: stratégie "2 pages d'abord" (last + last-1), puis fallback complet si besoin
# 5) Confiance OCR: calculée seulement si un candidat montant est trouvé (évite image_to_data inutile)
...
class PageImageCache:
    ...
    def get_rgb(self, page_index: int, dpi: int) -> np.ndarray:
...
# =========================
# QR detect (ROI bottom)
# =========================
def detect_bottom_qr_candidate(img_rgb: np.ndarray, bottom_ratio: float = 0.55):
    """
    Retourne: (qr_text, qr_points, y_start)
    qr_points: np.ndarray shape (4,2) en coordonnées image complète
    """
    if not HAS_CV2:
...
def scale_qr_points(qr_points: np.ndarray, scale: float) -> np.ndarray:
    if qr_points is None:
        return None
    pts = np.array(qr_points, dtype=np.float32)
    pts *= float(scale)
    return pts
...
def ocr_image_rgb_conf_if_useful(img_rgb: np.ndarray, mode: str = "text", require_amount_candidate: bool = False):
    """
    Optimisation: calcule la conf (image_to_data) seulement si utile.
    - Si require_amount_candidate=True: on calcule conf seulement si un candidat montant est détecté dans le texte.
    """
    if not (TESSERACT_AVAILABLE and Image is not None):
        return "", None
    try:
        ...
        if require_amount_candidate:
            # Pas de candidat => pas de conf => gain temps
            if not _AMOUNT_RX.search(normalize_spaces(txt or "")):
                return txt, None
...
def crop_amount_regions_invoice(img_rgb: np.ndarray):
    h, w, _ = img_rgb.shape
    crops = []
    crops.append(img_rgb[int(h * 0.62): h, 0:w])
    crops.append(img_rgb[int(h * 0.25): int(h * 0.78), int(w * 0.52): w])
    crops.append(img_rgb[int(h * 0.42): int(h * 0.70), int(w * 0.45): w])
    crops.append(img_rgb[int(h * 0.50): h, int(w * 0.50): w])
    return [c for c in crops if c.size > 0]


def crop_date_regions_top_third(img_rgb: np.ndarray):
    h, w, _ = img_rgb.shape
    y_end = int(h * (1.0 / 3.0))
    top = img_rgb[0:y_end, 0:w]
    crops = [top, top[:, int(w * 0.45): w], top[int(y_end * 0.15): y_end, 0:w]]
    return [c for c in crops if c.size > 0]


# =========================
# Amount/date extraction (v27 logique)
# =========================
...
def extract_total_amount_from_ocr_text(text: str):
    ...
    for line in lines:
        low = line.lower()
        base_penalty = -1 if ("tva" in low and "total" not in low) else 0

        m_ca = _CCY_AMOUNT_RX.search(line)
        if m_ca:
            curr = m_ca.group(1).upper()
            amt = parse_amount_extended(m_ca.group(2))
            score = 3 + base_penalty
            if "total" in low:
                score += 6
            if "ttc" in low:
                score += 3
            if low.startswith(curr.lower()):
                score += 1
            add(amt, curr, score)
...
def extract_invoice_total_from_page(img_rgb: np.ndarray):
    """
    v30: conf calculée seulement si on a un candidat montant (gain temps)
    """
    if not (TESSERACT_AVAILABLE and Image is not None):
        return None, None, None
    ...
    for crop in crop_amount_regions_invoice(img_rgb):
        txt, conf = ocr_image_rgb_conf_if_useful(crop, mode="amount", require_amount_candidate=True)
    ...
    return best_amt, best_ccy, best_conf
...
def extract_invoice_date_from_top(img_rgb: np.ndarray):
    if not (TESSERACT_AVAILABLE and Image is not None):
        return None
    ...
    for crop in crop_date_regions_top_third(img_rgb):
        txt = ocr_image_rgb(crop, mode="date")
        if not txt:
            continue
        ...
        for line in lines:
            ...
            penalty = -6 if ("échéance" in low or "echeance" in low) else 0
            boost = 0
            if "ins" in low:
                boost += 6
            if " le " in f" {low} ":
...
def compute_invoice_meta_v30(df_invoices: pd.DataFrame, cache: PageImageCache, dpi: int, read_amount: bool, read_date: bool):
    """
    v30: Montant "2 pages d'abord" => page_end puis page_end-1, puis fallback (qualité conservée)
    """
    inv = df_invoices.copy()

    amounts, currencies = [], []
    amount_sources, amount_confs = [], []
    dates, date_sources = [], []
    ...
    for _, row in inv.iterrows():
        ...
        if has_bv and bv_amt is not None:
            ...
            for p in candidates:
                try:
                    img = cache.get_rgb(page_index=p - 1, dpi=dpi)
                except Exception:
                    continue
                amt, ccy, conf = extract_invoice_total_from_page(img)
                if amt is not None:
                    inv_amt = float(amt)
                    inv_ccy = ccy
                    inv_amt_src = "FACT_OCR_NO_BV" if not has_bv else "FACT_OCR_BV_NO_AMOUNT"
                    inv_amt_conf = (float(conf) / 100.0) if conf is not None else None
                    found = True
                    break

            # 2) Fallback full scan (comme v27) si nécessaire
            if not found:
                for p in pages[::-1]:
                    if p in candidates:
                        continue
        ...
        if read_date and TESSERACT_AVAILABLE:
            try:
                img_first = cache.get_rgb(page_index=page_start - 1, dpi=dpi)
                inv_date = extract_invoice_date_from_top(img_first)
                inv_date_src = "FACT_OCR_TOP_1_3" if inv_date else None
            except Exception:
                inv_date = None
                inv_date_src = None
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### Paramètres (v27 après validation)")
...
tmp_dir = Path("data/tmp_bv_v30_com")
...
cache = PageImageCache(doc)

# ==========================================================
# PHASE 1 : scan rapide (si nouveau PDF)
# ==========================================================
...
if "df_pages_editable" not in st.session_state:
    ...
    for i in range(n_pages):
        img_rgb = cache.get_rgb(page_index=i, dpi=dpi_fast)
        ...
        row = {
            ...
            "has_bv": bool(has_qr_candidate),  # proxy FAST uniquement
            "bv_reference": None,
            "ref_source": None,
            "bv_currency": None,
            "bv_amount": None,
...
# ==========================================================
# PHASE 2 : strict (accéléré) seulement si validation
# ==========================================================
if validate_seg:
    ...
    scale_400_from_detect = float(dpi) / float(dpi_detect) if float(dpi_detect) > 0 else 1.0

    for i in range(n_pages):
        # 1) DPI_detect: QR detect+decode
        img_detect = cache.get_rgb(page_index=i, dpi=dpi_detect)
        ...
        # YOLO (sur dpi_detect pour limiter coût)
        tampon_pred = None
        proba_tampon = None
        if use_yolo and model is not None:
        ...
        if not has_qr_candidate:
            row = {
                "page": i + 1,
                "has_qr_candidate": False,
                "has_reference_strict": False,
                "has_bv": False,
                "bv_reference": None,
                "ref_source": None,
        ...
        # 2) Extraction monnaie+montant depuis payload (gratuit)
        bv_currency, bv_amount = (None, None)
        if qr_text:
            bv_currency, bv_amount = extract_currency_amount_from_qr_text(qr_text)

        # 3) Référence: essayer QRR payload en premier (0 OCR si OK)
        bv_reference = None
        ref_source = None
        if qr_text:
        ...
        # 4) OCR fallback seulement si pas trouvé via payload
        if bv_reference is None:
            # Render DPI 400 seulement ici (cache)
            img_400 = cache.get_rgb(page_index=i, dpi=dpi)

            # utiliser points scalés (évite re-détection en DPI 400)
            qr_pts_400 = scale_qr_points(qr_pts_detect, scale_400_from_detect)

            if qr_pts_400 is not None:
                for crop_img in crop_reference_regions(img_400, qr_pts_400, bottom_ratio=ocr_bottom_ratio):
    ...
    if df_final_invoices.shape[0] > 0:
        df_final_invoices = compute_invoice_meta_v30(
            df_final_invoices,
            cache=cache,
            dpi=dpi,
...
# ==========================================================
# AFFICHAGE strict v27 (identique à ton v27)
# ==========================================================
if "df_final_pages" in st.session_state and "df_final_invoices" in st.session_state:
    ...
    with d1:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v30_pages_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d2:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v30_factures_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d3:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v30_qr_ignores_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d4:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v30_transfert_erp_{pdf_path.stem}.csv",
            use_container_width=True,
        )
