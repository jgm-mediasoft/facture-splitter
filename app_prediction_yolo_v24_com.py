# app_prediction_yolo_v24_com.py
...
tmp_dir = Path("data/tmp_bv_v24_com")
...
edited_df = st.data_editor(
    ...
    column_config={
        ...
        "bv_amount": st.column_config.NumberColumn("Montant", format="%.2f"),
    },
    disabled=[c for c in cols if c != "invoice_start"],
    key="editor_pages_v24_com",
)
...
if "df_final_pages" in st.session_state and "df_final_invoices" in st.session_state:
    ...
    with d1:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v24_pages_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d2:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v24_factures_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d3:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v24_qr_ignores_{pdf_path.stem}.csv",
            use_container_width=True,
        )
