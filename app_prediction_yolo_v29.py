# app_prediction_yolo_v29_com.py
# v29 = v28 + accélération sans compromis qualité
# - PDF ouvert une seule fois (doc partagé)
# - Phase 2: scan "détection QR" à DPI bas, puis rerender DPI v27 uniquement si QR candidate
# - Le reste = v27 strict après validation (UI/exports identiques)
...
# -----------------------------
# RENDER (doc déjà ouvert)
# -----------------------------
def render_doc_page_to_rgb(doc: fitz.Document, page_index: int, dpi: int = 300):
...
def ocr_image_rgb_with_conf(img_rgb: np.ndarray, mode: str = "text"):
    if not (TESSERACT_AVAILABLE and Image is not None):
        return "", None
    try:
        pil_img = Image.fromarray(img_rgb)
        ...
        if mode in ("amount", "date"):
            ...
            df = pytesseract.image_to_data(pil_img, lang=lang, output_type=pytesseract.Output.DATAFRAME)
...
def extract_invoice_total_from_page(img_rgb: np.ndarray):
    ...
    for crop in crop_amount_regions_invoice(img_rgb):
        txt, conf = ocr_image_rgb_with_conf(crop, mode="amount")
...
def compute_invoice_meta(df_invoices: pd.DataFrame, doc: fitz.Document, dpi: int, read_amount: bool, read_date: bool):
    ...
    for _, row in inv.iterrows():
        ...
        if has_bv and bv_amt is not None:
            ...
            for p in pages[::-1]:
                try:
                    img_rgb = render_doc_page_to_rgb(doc, page_index=p - 1, dpi=dpi)
                except Exception:
                    continue
                amt, ccy, conf = extract_invoice_total_from_page(img_rgb)
                if amt is not None:
                    inv_amt = float(amt)
                    inv_ccy = ccy
        ...
        if read_date and TESSERACT_AVAILABLE:
            try:
                img_first = render_doc_page_to_rgb(doc, page_index=page_start - 1, dpi=dpi)
...
with st.sidebar:
    ...
    # Nouveau : DPI de détection (phase2) = rapide, sans OCR
    st.divider()
...
tmp_dir = Path("data/tmp_bv_v29_com")
...
# ==========================================================
# ÉTAPE 1 (FAST) : df_pages_editable sans OCR
# ==========================================================
if "df_pages_editable" not in st.session_state or st.session_state.get("last_pdf_name") != uploaded_pdf.name:
    ...
    for i in range(n_pages):
        img_rgb = render_doc_page_to_rgb(doc, page_index=i, dpi=dpi_fast)
...
# ==========================================================
# UI STRUCTURE (FAST) : édition invoice_start uniquement
# ==========================================================
...
# ==========================================================
# ÉTAPE 2 (STRICT v27) : optimisée mais résultats identiques
# ==========================================================
if recalc:
    progress = st.progress(0, text="Analyse…")
    rows = []

    # starts utilisateur depuis phase 1 (fast)
    user_starts = st.session_state["df_pages_editable"]["invoice_start"].fillna(False).astype(bool).values

    for i in range(n_pages):
        # 1) Détection QR candidate à DPI bas (rapide)
        img_detect = render_doc_page_to_rgb(doc, page_index=i, dpi=dpi_detect)
        ...
        _, qr_pts_detect, _ = detect_bottom_qr_candidate(img_detect, bottom_ratio=qr_bottom_ratio)
        has_qr_candidate = qr_pts_detect is not None

        # 2) Si pas de candidate -> on ne rerender pas en DPI 400, on sort direct (v27 aurait aussi has_bv False)
        ...
        # 3) Candidate => rerender DPI v27 (qualité inchangée), puis logique v27 strict
        img_rgb = render_doc_page_to_rgb(doc, page_index=i, dpi=dpi)
    ...
    if df_final_invoices.shape[0] > 0:
        df_final_invoices = compute_invoice_meta(
            df_final_invoices,
            doc=doc,
            dpi=dpi,
...
# ==========================================================
# AFFICHAGE : STRICT v27 (identique)
# ==========================================================
if "df_final_pages" in st.session_state and "df_final_invoices" in st.session_state:
    ...
    if apply_inv_edits:
        ...
        if "invoice_id" in df_apply.columns and "invoice_id" in edited_invoices.columns:
            ...
            tmp = edited_invoices.copy()
            tmp = tmp.set_index("invoice_id", drop=False)
    ...
    with d1:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v27_pages_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d2:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v27_factures_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d3:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v27_qr_ignores_{pdf_path.stem}.csv",
            use_container_width=True,
        )
    with d4:
        st.download_button(
            ...
            mime="text/csv",
            file_name=f"scan_v27_transfert_erp_{pdf_path.stem}.csv",
            use_container_width=True,
        )

# on ferme le doc proprement (streamlit rerun => ok)
try:
    doc.close()
except Exception:
    pass
